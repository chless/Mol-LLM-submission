...
class Stage2DM(LightningDataModule):
    def __init__(
        ...
        if False:
            self.pretrain_dataset = MoleculeCaption(
                root + f"/pretrain/", text_max_len, self.prompt, args.filtered_cid_path
            )
            self.train_dataset = MoleculeCaption(
                root + f"/train/", text_max_len, self.prompt
            )
            self.val_dataset = MoleculeCaption(
                root + "/valid/", text_max_len, self.prompt
            )
            self.test_dataset = MoleculeCaption(
                root + "/test/", text_max_len, self.prompt
            )
        else:
        ...
        self.is_gal = args.llm_model.find("galactica") >= 0
