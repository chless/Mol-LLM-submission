...
class TrainCollater:
    ...
    def __call__(self, batch):
        ...
        ## deal with prompt
        smiles_prompt = [
            smiles_handler(p, self.mol_ph, self.is_gal)[0] for p in smiles_prompt
        ]
...
class Stage2CheBIDM(LightningDataModule):
    def __init__(
        ...
        self.train_dataset = CheBIDataset(
            root + f"/train.txt", text_max_len, self.prompt
        )
        self.val_dataset = CheBIDataset(
            root + "/validation.txt", text_max_len, self.prompt
        )
