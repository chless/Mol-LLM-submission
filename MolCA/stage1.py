...
def main(args):
    ...
    if len(args.devices.split(",")) > 1:
        strategy = strategies.DDPStrategy(
            find_unused_parameters=find_unused_parameters, start_method="spawn"
        )
    else:
        strategy = "auto"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gtm",
        action="store_true",
        help="use graph-text matching or not",
        default=True,
    )
    parser.add_argument(
        "--lm", action="store_true", help="use language modeling or not", default=True
    )
