...
# def load_ignore_mismatch(model, state_dict):Blip2Regression
#     keys = set(model.state_dict().keys())
#     extra_keys = set()
...
class Blip2Regression(pl.LightningModule):
    ...
    def __init__(self, args):
        ...
        if args.opt_model.find("galactica") >= 0:
            if args.task == "regression":
                self.blip2opt = Blip2OPT_Regression(
                    args.bert_name,
                    args.gin_num_layers,
                    args.gin_hidden_dim,
                    args.drop_ratio,
                ...
                self.blip2opt = Blip2OPT_Regression2(
                    args.bert_name,
                    args.gin_num_layers,
                    args.gin_hidden_dim,
                    args.drop_ratio,
    ...
    def test_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            predictions = self.blip2opt.predict(batch)
    ...
    def on_test_epoch_end(self):
        ...
        regression_losses = self.blip2opt.calculate_regression_loss(
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.global_rank == 0:
            ...
            ## fixme: I am not sure if the max length is the same as previous experiments
            regression_losses = self.blip2opt.calculate_regression_loss(
                all_predictions, all_targets.input_ids
            )
            for key, loss_item in regression_losses.items():
                self.log(key, float(loss_item), sync_dist=True)
